  ...
  db.Comment.findOne({name: req.body.name, articleId: req.body.articleId}, (err, doc)=> {
    if (doc && doc.address !== req.body.address) {
      ...
      if (/^@(.*):/.test(req.body.content)) {
        ...
        db.Comment.findOne({name: reviewer, articleId: req.body.ariticleId}, (err, doc)=> {
          const url = "https://www.xxx.cn" + req.body.curPath
          const replyEmail = doc.address
          const content = emailForm("欢迎来到我的博客", reviewer, req.body.name, "回复了你的评论", req.body.content, url)
          mail.send(replyEmail, "您在HYQ的博客中有一条新评论", content, res)
        })
      }
      new db.Comment(comment).save().then(()=> {
        const url = "http://www.xxx.cn" + req.body.curPath
        const content = emailForm("MyBlog Message", "站长", req.body.name, "评论了你的文章", req.body.content, url)
        mail.send("he1275765531@sohu.com", "您的博客有一条新评论", content, res)
        res.status(200).send('send eamil successFully')
      }).catch((err) => {
        console.log(err)
      })
...
router.get("./api/comments", (req, res)=> {
  ...
  if (req.query.payload.sort == "date") {// 根据时间排序评论
    db.Comment.find({articleId: articleId}, 'name date content like imgName').sort({date: -1}).exec().then((comments)=> {
      res.send(comments)
    })
  } else if (req.query.payload.sort === "like") {// 根据点赞数量排序评论
    db.Comment.find({articleId: articleId}, 'name date content like imgName').sort({like: -1}).exec().then((comments)=> {
      res.send(comments)
    })
  } else {// 根据文章的aid获取所有评论
