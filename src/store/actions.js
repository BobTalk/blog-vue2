...
export  default {
  ...
  resetUser({commit}, payload){
    return Vue.http.post("/api/user", payload).then(()=> {
      commit("unset_user")
  ...
  saveArticle({state, commit}, aid){
    ...
    if (aid) {
      return Vue.http.patch("/api/article/" + aid, state.article).then(()=> {
      ...
      return Vue.http.post("/api/article", state.article).then(()=> {
        commit("isSaving_toggle", true)
        router.push({name: "posts"})
      }, ()=> {
        alert("faile saving")
  ...
  getAllArticles({commit}, payload){
    ...
    return Vue.http.get("/api/articles", {params: {payload}}).then((res)=> {
      return res.json()
    }).then(articles=> {
      ...
      if (payload.add) {
        ...
        commit("set_all_article", articles)
  ...
  getArticle({commit, state}, aid){
    ...
    document.title = "加载中..."
    return Vue.http.get("/api/article/" + aid).then((res)=> {
